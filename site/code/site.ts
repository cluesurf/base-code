...
async function getDocumentKeywords(
  resource: string,
  ...
  if (!result) {
    result = parseKeywords(
      await fs.promises.readFile(url.fileURLToPath(resource), 'utf-8'),
    )
...
async function captureKeywords(
...
connection.onCompletion(
  (
    ...
    const keywords =
      documentKeywords.get(_textDocumentPosition.textDocument.uri) ?? []
